...
// Shell et PasswordInput définis EN DEHORS du composant de page (au niveau
// module) : les définir à l'intérieur les recrée à chaque frappe clavier,
// ce qui remonte tout le formulaire et casse le focus (bug déjà rencontré
// et corrigé sur la version client, corrigé ici dès le départ).
...
export function MoverCreateQuickLeadPasswordPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/get-mover-lead-verification`, {
          method: 'POST',
          headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-mover-lead-account`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY}`,
    ...
    return (
      <Shell>
        <div className="text-center">
          ...
          <a href="/inscription-demenageur" className="text-blue-600 font-medium text-sm">Refaire une demande →</a>
        </div>
      </Shell>
    );
  }
    ...
    return (
      <Shell>
        <div className="text-center">
          ...
          <a href="/mover/login" className="inline-block bg-blue-600 text-white font-semibold px-6 py-2.5 rounded-lg text-sm">
            Se connecter
          </a>
        </div>
      </Shell>
  ...
  return (
    <Shell>
      ...
      <p className="text-gray-500 text-sm text-center mb-6">
        Sécurisez le compte de {companyName || 'votre entreprise'} ({email}).
      </p>
