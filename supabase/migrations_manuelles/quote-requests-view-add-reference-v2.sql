-- Recrée la vue avec DROP puis CREATE (comme 20260201000002) plutôt que
-- CREATE OR REPLACE : la vue en base contient déjà accepts_groupage à une
-- position que CREATE OR REPLACE ne peut pas renommer/déplacer, ce qui
-- faisait échouer la migration. DROP+CREATE n'a pas cette contrainte.
...
'Secure view that automatically masks client contact information for movers until first payment is completed. Includes all fields from quote_requests including furniture_inventory, furniture_photos, carrying distances, groupage preferences, distance_km, market_price_estimate and reference.';
